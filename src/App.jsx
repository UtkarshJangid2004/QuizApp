...
const Quiz = () => {
  ...
  const handleNextQuestion = () => {
    ...
    if (currentQuestionIndex < shuffledQuestions.length - 1) {
      ...
      updatePlayerScore(); // Update score before marking quiz as finished
    }
  };

  const updatePlayerScore = () => {
    const updatedPlayers = players.map((player) =>
      player.name === playerName ? { ...player, score: score } : player
    );
    ...
    return (
      <>
        ...
        <div className="w-full h-screen flex items-center justify-center">
          <div className="quiz-container h-[80vh] w-[50vw] ml-[7vw]">
            <h1 className="text-5xl  mb-5">Quiz App</h1>
            <h2>Your score is: {score}</h2>
          </div>
          <Leaderboard playerName={playerName} /> {/* Pass playerName here */}
        </div>
      </>
    );
  }
